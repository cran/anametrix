...
getAxDataset <- function(auth, reportSuiteId, tableConfigurationXML, columns, datestart, dateend, count, verbose) {
	if(is.null(auth$axapiUri) || as.character(auth$axapiUri) == "") {	
		cat("You need to specify API Endpoint URL first: newAuthentication <- authentication(apiURL,username,password)","\n") 
		return(NULL)
	}
...
getFullTableListXML <- function(auth, reportsuiteId) {
	...
	resp = getURL(auth$axapiUri, ssl.verifypeer = FALSE, postfields=tableListXMLreq, .opts = list(timeout = 10));
